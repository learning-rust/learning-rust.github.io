...
> [!recap]
> The `&` or `&mut` operators are used for borrowing and `*` operator for dereferencing. We will discuss them with 
> [Ownership](/docs/ownership), [Borrowing](/docs/borrowing) & [Lifetimes](/docs/lifetimes). Please focus on operators first.
...
> - Also, we will discuss with more examples under [`impl`](/docs/impls) and [`trait`](/docs/traits).

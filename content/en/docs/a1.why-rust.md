...
- It doesn't use a built-in runtime or an automated garbage collection system \(GC\).
    ...
    > [!recap]
    > However, async Rust requires an async runtime, which is provided by community-maintained crates like [`tokio`](https://github.com/tokio-rs/tokio), [`smol`](https://github.com/smol-rs/smol) etc. The async runtime will be bundled into the final executable.
